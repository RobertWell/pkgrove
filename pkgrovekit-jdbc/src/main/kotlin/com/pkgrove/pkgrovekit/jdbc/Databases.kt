 ...
 * (HEL-128). The core rule: PkgroveKit borrows resources for a declared scope,
 ...
 *  - PKGROVEKIT_MANAGED: we own it (tests, CLI, file DuckDB); [Databases] is
 ...
 *    committed/closed by PkgroveKit (see TransactionPolicy).
 */
...
class Databases private constructor(
    ...
    enum class Ownership { APPLICATION_OWNED, PKGROVEKIT_MANAGED }
    ...
    /** Close PKGROVEKIT_MANAGED resources (reverse registration order,
    ...
    override fun close() {
        ...
        for (k in order.reversed()) {
            val e = entries.getValue(k)
            if (e.ownership == Ownership.PKGROVEKIT_MANAGED &&
    ...
    class Builder internal constructor() {
         ...
         *  transaction state cannot be recovered; without it PkgroveKit falls
        ...
        /** Register a PKGROVEKIT-MANAGED resource with its closer. */
        @JvmOverloads
        fun managed(key: DatabaseKey, dataSource: DataSource, closer: () -> Unit,
                    dialect: SqlDialect? = null, maxConnections: Int = 4,
                    acquisitionTimeoutMillis: Long = 30_000,
                    invalidator: ((Connection) -> Unit)? = null) {
            register(Entry(key, dataSource, Ownership.PKGROVEKIT_MANAGED, dialect,
