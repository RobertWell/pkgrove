...
class OracleTransferIT {
    ...
    fun `duckdb to oracle via jdbi transfer facade honors the caller transaction`() {
        freshDuck().use { duck ->
            ...
            val report = jdbi.withHandle<com.pkgrove.pkgrovekit.core.OperationReport, Exception> { h ->
                h.inTransaction<com.pkgrove.pkgrovekit.core.OperationReport, Exception> { txh ->
                    JdbiTransfer.run(duck, "SELECT * FROM jsrc", emptyMap(),
                        txh, OracleDialect, "jdbi_dest",
                        Transfer.Options(mode = SqlDialect.TargetMode.APPEND, mapping = mapping))
                }
    ...
    private val txSchema = com.pkgrove.pkgrovekit.core.Schema(listOf(
        com.pkgrove.pkgrovekit.core.Column("id", com.pkgrove.pkgrovekit.core.ValueKind.NUMERIC, "NUMBER", precision = 9)))
    private fun txBatch(values: List<Long?>) = com.pkgrove.pkgrovekit.core.RowBatch(
        txSchema, values.map { com.pkgrove.pkgrovekit.core.Row(txSchema, listOf(it)) })
    ...
    fun `savepoint-per-batch on oracle keeps earlier batches and skips the poison one`() {
        ...
        val o = com.pkgrove.pkgrovekit.jdbc.TransactionalWriter.write(
            oconn, "INSERT INTO sp_dest VALUES (?)", batches,
            com.pkgrove.pkgrovekit.jdbc.TransactionPolicy.SavepointPerBatch, OracleDialect)
        assertEquals(com.pkgrove.pkgrovekit.jdbc.TransactionState.PARTIALLY_COMMITTED, o.state)
    ...
    fun `atomic policy on oracle rolls the whole transfer back on a poison row`() {
        ...
        val ex = org.junit.jupiter.api.Assertions.assertThrows(
            com.pkgrove.pkgrovekit.jdbc.TransactionWriteException::class.java) {
            com.pkgrove.pkgrovekit.jdbc.TransactionalWriter.write(
                oconn, "INSERT INTO atomic_dest VALUES (?)", batches,
                com.pkgrove.pkgrovekit.jdbc.TransactionPolicy.Atomic, OracleDialect)
        }
        assertEquals(com.pkgrove.pkgrovekit.jdbc.TransactionState.ROLLED_BACK, ex.outcome.state)
        assertEquals(0L, ex.outcome.committedRows)
        assertEquals(com.pkgrove.pkgrovekit.jdbc.RetrySafety.SAFE_NOTHING_COMMITTED, ex.outcome.retrySafety)
        oconn.createStatement().use { st ->
